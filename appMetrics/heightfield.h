...
struct PathNode {
    int x, y;       // Coordonnées grille
    double gCost;   // Coût depuis le départ
    double hCost;   // Heuristique (dist vers arrivée)
    double fCost;   // g + h
    int parentX, parentY; // Pour reconstruire le chemin

    // Comparateur pour la priority_queue (on veut le plus petit fCost en haut)
...
public:
    ...
    // Erosion
    // EXO TP
    void thermalErode(int iterations = 200, double talusAngleDeg = 30.0, double rate = 0.35, bool useDiagonals = true);
    ...
    // slope magnitude s(p) from height h
    ScalarField2 slopeField(bool useDiagonals = true) const;

    // D8 drainage area / flow accumulation A(p) (number of upstream cells, incl. self)
    ScalarField2 drainageAreaD8() const;

    // wetness index w(p) = ln( A(p) / s(p) )   (slides)
    ScalarField2 wetnessIndex(const ScalarField2& A, const ScalarField2& s) const;

    // simple sunlight proxy L(p) = clamp(n . sun, 0, 1)
    ScalarField2 lightField(double sunAzimuthDeg = 315.0, double sunAltitudeDeg = 45.0) const;

    // suitability/probability p : R^2 -> [0,1] = min_k r_k∘e_k   (slides)
    ScalarField2 vegetationSuitability(const ScalarField2& slope,
                                    const ScalarField2& wetness,
                                    const ScalarField2& light,
                                    double slopeMaxDeg = 35.0,
                                    double wetnessMu = 0.0,
                                    double wetnessSigma = 1.0) const;

    // one simulation step on density V in [0,1]
    void vegetationStep(ScalarField2& V,
    ...
    // initialize density with random seeding proportional to P
    void vegetationSeed(ScalarField2& V, const ScalarField2& P, double seedProb = 0.02, uint32_t seed = 1) const;
            
    std::vector<Vector3> findRoadPath(const Vector3& startWorld, const Vector3& endWorld, double slopeWeight) const;

    ScalarField2 computeSnowCover(double snowAltitude, double steepnessThreshold) const;

    void applyBrush(const Vector3& p, double radius, double intensity);
    
    void applyRoadTerraforming(const std::vector<Vector3>& path, 
                               double roadWidth, 
                               double blendWidth,
                               ScalarField2* vegetation = nullptr);
...
protected:
    ...
    double computeSlopeCost(int x1, int y1, int x2, int y2, double slopeWeight) const;
