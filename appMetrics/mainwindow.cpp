    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(trafficTimer, SIGNAL(timeout()), this, SLOT(updateTrafficSimulation()));
}
...
void MainWindow::createActions()
{
    ...
    // EXO TP
    connect(ui->btn_thermalErosion, SIGNAL(clicked()), this, SLOT(editThermalErosion()));
    connect(ui->btn_hydraulicErosion, SIGNAL(clicked()), this, SLOT(editHydraulicErosion()));
    // connect(ui->btn_vegCompute, SIGNAL(clicked()), this, SLOT(editVegetationCompute()));
    // connect(ui->btn_vegStep,    SIGNAL(clicked()), this, SLOT(editVegetationStep()));
    // connect(ui->btn_vegReset,   SIGNAL(clicked()), this, SLOT(editVegetationReset()));
    connect(ui->btn_vegCompute, &QPushButton::clicked, this, [this]() { editVegetationCompute(); });
    connect(ui->btn_vegStep,    &QPushButton::clicked, this, [this]() { editVegetationStep(); });
    connect(ui->btn_vegReset,   &QPushButton::clicked, this, [this]() { editVegetationReset(); });
    
    connect(ui->cb_display_snow, SIGNAL(clicked()), this, SLOT(updateTexture()));
    connect(ui->sb_snow_altitude, SIGNAL(valueChanged(double)), this, SLOT(updateTexture()));

    connect(ui->btn_build_road, &QPushButton::clicked, this, [this]() {
        if (currentRoadPath.empty()) {
            setStatusText("Erreur: Aucune trajectoire calculée !");
            return;
        }

        setStatusText("Construction de la route (Terraforming)...");

        double width = ui->sb_road_width->value();
        double blend = ui->sb_road_blend->value();

        ScalarField2* vegPtr = vegInitialized ? &vegV : nullptr;
        
        hf.applyRoadTerraforming(currentRoadPath, width, blend, vegPtr);

        updateHeightfield(false); 
        setStatusText("Route construite !");
    });
    connect(ui->btn_gen_network, &QPushButton::clicked, this, &MainWindow::generateRoadNetwork);
...
// EXO TP
void MainWindow::editThermalErosion()
{
    // hf.thermalErode(200, 30.0, 0.35, true);
    hf.thermalErode(20, 80.0, 0.5, true);
    updateHeightfield(false);
}

void MainWindow::editHydraulicErosion()
{

    int iterations      = ui->sb_hydro_iter->value();
    double rain         = ui->dsb_hydro_rain->value();
    double evaporation  = ui->dsb_hydro_evap->value();
    double flowRate     = ui->dsb_hydro_flow->value();
    double capacity     = ui->dsb_hydro_cap->value();
    double erosionRate  = ui->dsb_hydro_ero->value();
    double deposition   = ui->dsb_hydro_dep->value();
    bool useDiagonals   = ui->cb_hydro_diag->isChecked();

    setStatusText("Simulation hydraulique en cours (" + QString::number(iterations) + " itérations)...");
    ...
    std::cout << "--- Résultats Erosion ---" << std::endl;
    std::cout << "Gouttes/Itérations : " << iterations << std::endl;
    std::cout << "Temps calcul : " << ms << " ms" << std::endl;
    
    setStatusText("Erosion terminée en " + QString::number(ms) + " ms");

    updateHeightfield(false);
}

void MainWindow::editVegetationReset()
{
    // vegP = ScalarField2(hf.getSizeX(), hf.getSizeY());
    // vegV = ScalarField2(hf.getSizeX(), hf.getSizeY());
    // for(int i=0;i<hf.getSizeX();++i) for(int j=0;j<hf.getSizeY();++j) { vegP(i,j)=0.0; vegV(i,j)=0.0; }
    // vegInitialized = false;
    // updateHeightfield(false);
    vegP = ScalarField2(hf.getDomain(), hf.getSizeX(), hf.getSizeY(), 0.0);
    vegV = ScalarField2(hf.getDomain(), hf.getSizeX(), hf.getSizeY(), 0.0);
    vegInitialized = false;
    updateHeightfield(false);

}

void MainWindow::editVegetationCompute()
{
    // 1) abiotic maps
    ScalarField2 slope = hf.slopeField(true);
    ScalarField2 A     = hf.drainageAreaD8();
    ScalarField2 wet   = hf.wetnessIndex(A, slope);                 // w = ln(A/s) :contentReference[oaicite:4]{index=4}
    ScalarField2 light = hf.lightField(315.0, 45.0);                // simple sun proxy

    // 2) probability p = min_k r_k∘e_k :contentReference[oaicite:5]{index=5}
    vegP = hf.vegetationSuitability(
        slope, wet, light,
        ui->sb_vegSlopeMax->value(),      // deg
        ui->dsb_vegWetMu->value(),        // mu
        ui->dsb_vegWetSigma->value()      // sigma
    );

    // init vegetation if not done
    if (!vegInitialized) {
        // vegV = ScalarField2(hf.getSizeX(), hf.getSizeY());
        vegV = ScalarField2(hf.getDomain(), hf.getSizeX(), hf.getSizeY(), 0.0);
        for(int i=0;i<hf.getSizeX();++i) for(int j=0;j<hf.getSizeY();++j) vegV(i,j)=0.0;
        hf.vegetationSeed(vegV, vegP, ui->dsb_vegSeedProb->value(), 1);
        vegInitialized = true;
    }
...
void MainWindow::editVegetationStep()
{
    if (!vegInitialized) editVegetationCompute();

    const int steps = ui->sb_vegSteps->value();
    const double g  = ui->dsb_vegGrowth->value();
    const double d  = ui->dsb_vegDeath->value();
    const double D  = ui->dsb_vegDiffusion->value();

    for (int k = 0; k < steps; ++k)
        hf.vegetationStep(vegV, vegP, g, d, D, 1.0);
...
QImage MainWindow::composeTexture() const
{
    ...
    if (ui->cb_display_snow->isChecked())
    {
        double altitude = ui->sb_snow_altitude->value();
        double steepness = 0.5;

        ScalarField2 snowMap = hf.computeSnowCover(altitude, steepness);
        ...
        for (int x = 0; x < background.width(); ++x)
        {
            for (int y = 0; y < background.height(); ++y)
            {
                ...
                int gx = (int)((double)x / background.width() * hf.getSizeX());
                int gy = (int)((double)y / background.height() * hf.getSizeY());
                
                gx = std::max(0, std::min(gx, hf.getSizeX() - 1));
                gy = std::max(0, std::min(gy, hf.getSizeY() - 1));

                double snowVal = snowMap.at(gx, gy);

                if (snowVal > 0.01)
                {
                    QColor oldColor = background.pixelColor(x, y);
                    QColor snowColor(255, 255, 0); 

                    int r = oldColor.red()   * (1.0 - snowVal) + snowColor.red()   * snowVal;
                    int g = oldColor.green() * (1.0 - snowVal) + snowColor.green() * snowVal;
                    int b = oldColor.blue()  * (1.0 - snowVal) + snowColor.blue()  * snowVal;

                    background.setPixelColor(x, y, QColor(r, g, b));
                }
            }
        }
    }
    ...
    if (!currentRoadPath.empty()) {
        ...
        float scaleX = (float)background.width() / (float)hf.getSizeX();
        float scaleY = (float)background.height() / (float)hf.getSizeY();

        for (const Vector3& v : currentRoadPath) {
            Index2 c = hf.cellCoords(Vector2(v));
            points.push_back(QPointF(c.x() * scaleX, c.y() * scaleY));
        }

        double metersToPixels = 1.0 / hf.getCellSize()[0] * scaleX;

        double roadWidthPixels = ui->sb_road_width->value() * metersToPixels; 
        
        roadWidthPixels = std::max(2.0, roadWidthPixels);

        p.setPen(QPen(QColor(60, 60, 60), roadWidthPixels, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
    ...
    if (!cities.empty()) {
        ...
        for (const Vector3& city : cities) {
            Index2 c = hf.cellCoords(Vector2(city));
            p.drawEllipse(QPointF(c.x() * scaleX, c.y() * scaleY), 10, 10);
        }
        p.end();
    }
...
void MainWindow::queryRay(const Ray& ray)
{
    ...
    if (hf.Intersect(ray, t, p, hf.getBox(), 1)) {
        ...
        if (ui->btn_tool_sculpt->isChecked()) {

             widget->setCursorStyle(Norm(hf.getCellSize()), Vector3(1.0, 0.0, 0.0));
        } else {
             widget->setCursorStyle(Norm(hf.getCellSize()), Vector3(0.0, 0.7, 0));
        }
        ...
        if (ui->btn_tool_sculpt->isChecked()) {
            ...
            double radius = ui->sb_brush_radius->value();
            double intensity = ui->sb_brush_intensity->value();
            
            if (ui->cb_sculpt_dig->isChecked()) {
                intensity = -std::abs(intensity);
            } else {
                intensity = std::abs(intensity);
            }

            hf.applyBrush(p, radius, intensity);

            updateHeightfield(false);
            
            std::cout << "Sculpt at " << p[0] << "," << p[1] << " | Val: " << intensity << std::endl;
            
            return;
        }
        if (ui->btn_tool_road->isChecked()) {
            if (selectingRoadStart) {
                roadStartPoint = p;
                setStatusText("Point de départ route défini. Cliquez pour la fin.");
                selectingRoadStart = false;
            } else {
                roadEndPoint = p;
                setStatusText("Calcul de la route...");
                
                double slopeWeight = 1000.0;
                currentRoadPath = hf.findRoadPath(roadStartPoint, roadEndPoint, slopeWeight);
                
                drawRoadOnTexture(); 
                
                selectingRoadStart = true; 
            }
        }
    }
    else {
...
void MainWindow::drawRoadOnTexture() {
    if (currentRoadPath.empty()) return;

    QImage tex = composeTexture(); 
    
    float scaleX = (float)tex.width() / (float)hf.getSizeX();
    float scaleY = (float)tex.height() / (float)hf.getSizeY();
    ...
    for (size_t i = 0; i < currentRoadPath.size() - 1; ++i) {
        Vector3 p1 = currentRoadPath[i];
        Vector3 p2 = currentRoadPath[i+1];

        Index2 c1 = hf.cellCoords(Vector2(p1));
        Index2 c2 = hf.cellCoords(Vector2(p2));

        painter.drawLine(QPointF(c1.x() * scaleX, c1.y() * scaleY), 
                         QPointF(c2.x() * scaleX, c2.y() * scaleY));
    }
...
void MainWindow::generateRoadNetwork()
{
    int numCities = 5;      
    cities.clear();
    currentRoadPath.clear(); 

    std::cout << "Génération de " << numCities << " villes..." << std::endl;

    int attempts = 0;
    
    while (cities.size() < (size_t)numCities && attempts < 1000) {
        attempts++;
        
        int i = rand() % (int)(hf.getSizeX() * 0.8) + (hf.getSizeX() * 0.1);
        int j = rand() % (int)(hf.getSizeY() * 0.8) + (hf.getSizeY() * 0.1);
        
        double h = hf.at(i, j);
        Vector3 n = hf.Normal(i, j);
        double slope = 1.0 - n[2]; // Calcul manuel de la pente
        
        if (h < 0.0) continue; 
        if (slope > 0.1) continue;     
        
        bool tooClose = false;
        Vector3 candidate = hf.Vertex(i, j);
        
        for (const auto& c : cities) {
            Vector3 diff = c - candidate;
        ...
        cities.push_back(candidate);
    }

    if (cities.size() < 2) {
        setStatusText("Pas assez d'endroits valables pour des villes.");
        return;
    }

    std::vector<Vector3> fullNetworkPath;

    for (size_t i = 0; i < cities.size(); ++i) {
        Vector3 start = cities[i];
        Vector3 end = cities[(i + 1) % cities.size()]; 

        std::vector<Vector3> segment = hf.findRoadPath(start, end, 500.0);
        
        fullNetworkPath.insert(fullNetworkPath.end(), segment.begin(), segment.end());
    }

    currentRoadPath = fullNetworkPath;

    ScalarField2* vegPtr = vegInitialized ? &vegV : nullptr;
    hf.applyRoadTerraforming(currentRoadPath, 100.0, 100.0, vegPtr);

    updateHeightfield(false); 
    widget->update();
    
    setStatusText("Réseau routier généré et construit !");

    spawnCars();
}

void MainWindow::spawnCars() {
    ...
    for(int i=0; i<20; ++i) {
        ...
        int startCityIdx = rand() % cities.size();
        int endCityIdx = rand() % cities.size();
        while(endCityIdx == startCityIdx) endCityIdx = rand() % cities.size();

        car.path = hf.findRoadPath(cities[startCityIdx], cities[endCityIdx], 200.0);
    ...
    trafficTimer->start(33); 
}

void MainWindow::updateTrafficSimulation() {
    ...
    for (auto& car : traffic) {
        ...
        if (car.forward) {
            car.pathIndex += speed;
            if (car.pathIndex >= (int)car.path.size()) {
                car.pathIndex = car.path.size() - 1;
                car.forward = false; 
            }
        } else {
            car.pathIndex -= speed;
        ...
        car.pos = car.path[car.pathIndex];
    }
    ...
    for (const Agent& car : traffic) {
        Index2 c = hf.cellCoords(Vector2(car.pos));
        
        p.drawEllipse(QPointF(c.x() * scaleX, c.y() * scaleY), 5, 5);
    }
    p.end();
